...
export default function DashboardClient({ user }: { user: User }) {
  ...
  const referralLink = `https://uniquevoyage.site?ref=${user.id.slice(0, 8)}`;
        ...
        {!user.is_vip && (
          ...
          >
            ...
            <div className="rounded-2xl border border-[#D85A30]/30 bg-[#D85A30]/5 p-6 md:p-8 space-y-5">
              ...
              <button
                onClick={handleCopy}
                className="group flex w-full items-center justify-between rounded-xl border border-white/10 bg-black/30 px-5 py-4 text-left transition-all hover:border-[#D85A30]/40 hover:bg-[#D85A30]/5"
              >
